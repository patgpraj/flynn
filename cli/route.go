...
usage: flynn route
       flynn route add http [-s <service>] [-c <tls-cert> -k <tls-key>] [--sticky] [--leader] [--no-leader] [--drain-backends] <domain>
       flynn route add tcp [-s <service>] [-p <port>] [--leader] [--drain-backends]
...
Options:
	...
	--drain-backends           wait for in-flight requests to complete before stopping backends
...
func runRouteAddTCP(args *docopt.Args, client controller.Client) error {
	...
	hr := &router.TCPRoute{
		...
		DrainBackends: args.Bool["--drain-backends"],
	}
